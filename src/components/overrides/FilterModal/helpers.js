    ...
    for (const key of uniqueKeys) {
        ...
        if (filterKeys.has(key) && key.endsWith('[filter]')) {
            ...
            for (const value of params.getAll(key)) {
                // CUSTOM
                // check if price filter, then ignore existing options and create a new entry
                const existingFilter =
                    group === 'price'
                        ? { title: value.split(',')[0], value: value.split(',')[1] }
                        : groupItemsByValue.get(getValueFromFilterString(value));
                //
                if (existingFilter) {
