...
export const useProduct = props => {
    ...
    const handleRemoveFromCart = useCallback(async () => {
        try {
            ...
            trackRemoveFromCart({
                ...
                priceValue: item.prices.price.value,
                products: [
                    {
    ...
    }, [getProductCategories, item, trackProductClick]);
...
const flattenProduct = (item, configurableThumbnailSource, storeUrlSuffix) => {
    ...
    const { name, small_image, stock_status: stockStatus, url_key: urlKey, preorder_note } = product;
    const { url: image } =
