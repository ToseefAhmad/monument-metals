...
const makeOptimizedUrl = (path, { type, ...opts } = {}) => {
    ...
    // If CF optimization is not enabled (backend), add base url
    if (alreadyOptimized) {
    ...
    const newPath = `/cdn-cgi/image/${paramsString}/${normalizeSrc(baseURL.href.slice(baseURL.origin.length))}`;
    baseURL = new URL(newPath, mediaBackend);
